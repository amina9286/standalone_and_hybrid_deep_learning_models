# -*- coding: utf-8 -*-
"""new_Q+Rf+temp+if_GRU

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/13UGfdD-bTBT1DG3AAyRiDoXUH8MS1E8d
"""
...
for day in [1,2,3]:
  ...
  regressorGRU.add(GRU(units=128,input_shape=(x_train.shape[1],x_train.shape[2]), activation='relu'))
  regressorGRU.add(Dropout(0.1))
  # Second GRU layer
  #regressorGRU.add(GRU(units=50, return_sequences=True, input_shape=(x_train.shape[1],x_train.shape[2]), activation='relu'))
  ...
  GRU_history =regressorGRU.fit(x_train,y_train,validation_data=(x_val, y_val),epochs=100,batch_size=32)
  ...
  #predictions
  train_yp = regressorGRU.predict(x_train.reshape(len(x_train),1, x_train.shape[2],1))
  val_yp = regressorGRU.predict(x_val.reshape(len(x_val),1, x_val.shape[2], 1))
  test_yp =regressorGRU.predict(x_test.reshape(len(x_test),1, x_test.shape[2],1))
  ...
  #to save as csv file
  dataset.to_csv(f'/content/drive/MyDrive/new_ACF/GRU/Q+rf+temp+if/lead_day_{day}_gru_train_day.csv')
  ...
  dataset_test.to_csv(f'/content/drive/MyDrive/new_ACF/GRU/Q+rf+temp+if/lead_day_{day}_gru_test_day.csv')
  ...
  res['Stats'] = names
  res['Values'] = results
  res.to_csv(f'/content/drive/MyDrive/new_ACF/GRU/Q+rf+temp+if/lead_day_{day}_gru_result_day.csv')
